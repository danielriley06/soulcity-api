...
require_relative '../lib/cluster/lifecycle_events'
require_relative '../lib/cluster/puma_worker_killer_initializer'
...
#
threads_min = Rails.application.credentials.dig(Rails.env.to_sym, :RAILS_MIN_THREADS)
threads_max = Rails.application.credentials.dig(Rails.env.to_sym, :RAILS_MAX_THREADS)
...
on_restart do
  ...
  ::Cluster::LifecycleEvents.do_master_restart
end
...
before_fork do
  ...
  ::Cluster::PumaWorkerKillerInitializer.start @config.options unless ENV['DISABLE_PUMA_WORKER_KILLER']

  # Signal application hooks that we're about to fork
  ::Cluster::LifecycleEvents.do_before_fork
end
...
#
::Cluster::LifecycleEvents.set_puma_options @config.options
on_worker_boot do
  ::Cluster::LifecycleEvents.do_worker_start
